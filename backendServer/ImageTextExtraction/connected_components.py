...
def returnMaxValue(someVar):
  maxValue = max(someVar.stop-someVar.start,0)
  return maxValue
...
def area_bb(a):
  tempList = []
  for x in a[:2]:
    maxVar = returnMaxValue(x)
    tempList.append(maxVar)
  return multiplyList(tempList)

# def area_bb(a):
#   return multiplyList([max(x.stop-x.start,0) for x in a[:2]])
...
def findAreaOfComponent(component):
  return area_bb(component)**0.5
...
def bounding_boxes(image,connected_components,max_size,min_size):
  ...
  for component in connected_components:
    areaOfComponent = findAreaOfComponent(component)
    if areaOfComponent<min_size: continue
    if areaOfComponent>max_size: continue
...
def draw_bounding_boxes(img,connected_components,max_size=0,min_size=0,color=(0,0,255),line_size=2):
  ...
  for component in connected_components:
    areaOfComponent = findAreaOfComponent(component)
    if min_size > 0 and areaOfComponent<min_size: continue
    if max_size > 0 and areaOfComponent>max_size: continue
    #a = area_nz(component,img)
    #if a<min_size: continue
    #if a>max_size: continue
    (ys,xs)=component[:2]
    #print([xs.start,ys.start,xs.stop,ys.stop])
    coordinatesList.append([xs.start,ys.start,xs.stop,ys.stop])
    #cv2.rectangle(img,(xs.start,ys.start),(xs.stop,ys.stop),color,line_size)
  #jsonList = json.dumps(coordinatesList)
